...
import '../viewmodels/standard_transfer_model.dart';

class StandardTemplateWidget extends StatelessWidget {
  StandardTemplateWidget({
    Key? key,
  }) : super(key: key);

  final List<StandardTransferModel> _standardTransferModel = [
    StandardTransferModel(
      icon: AppAssets.standardTransfer,
      title: 'Aysel',
      subtitle: '**** **** **** **** 3724',
      amount: '10.00 ₼',
    ),
      ...
      child: ListView(
        children: [
          ...
          ListView.separated(
            ...
            shrinkWrap: true,
            itemCount: _standardTransferModel.length,
            itemBuilder: (context, index) {
              final String amount = _standardTransferModel[index].amount;
  ...
  Padding _buildTitle(int index) => Padding(
        ...
        child: Text(
          _standardTransferModel[index].title,
  ...
  GestureDetector _buildSubtitle(int index) => GestureDetector(
        child: Text(
          _standardTransferModel[index].subtitle,
  ...
  Container _buildLeading(BuildContext context, int index) => Container(
        ...
        child: SvgPicture.asset(
          _standardTransferModel[index].icon,
          fit: BoxFit.scaleDown,
        ),
      );

  Padding _buildTrailing(List chunks) => Padding(
